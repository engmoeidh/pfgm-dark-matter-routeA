import os, json, yaml, numpy as np, emcee
...
def main():
    ...
    oneh        = load_routeA_1h(man["paths"]["routeA_1h_binB"], R) if "routeA_1h_binB" in man["paths"] else load_routeA_1h(man["paths"]["routeA_1h_binA"], R)
    k, Pnl      = build_pnl(man["paths"]["p_nl"], man["fitting"]["k_min"], man["fitting"]["k_max"], man["fitting"]["n_k"])
    # start near LSQ if available
    p0 = np.array([1.0,1.6,10.0,1.1])
    fitB = "results/tables/bin_B_fit_freeA.json"
    if os.path.isfile(fitB):
        d=json.load(open(fitB,"r"))
        p0 = np.array([d["A1h"], d["b0"], d["b2"], d["lam"]], dtype=float)
    nwalk, ndim, steps = 32, 4, 1000
    ...
    os.makedirs("results/posteriors", exist_ok=True); np.save("results/posteriors/binB_freeA_chain_quick.npy", chain)
    qs = np.percentile(chain, [16,50,84], axis=0)
    out = {k:{"p16":float(qs[0,i]),"p50":float(qs[1,i]),"p84":float(qs[2,i])} for i,k in enumerate(["A1h","b0","b2","lam"])}
    os.makedirs("results/tables", exist_ok=True); json.dump(out, open("results/tables/binB_mcmc_freeA.json","w"), indent=2)
    print("λ_B (p50 ±1σ) ≈", out["lam"]["p50"], "+/-", 0.5*(out["lam"]["p84"]-out["lam"]["p16"]))
